...
#define FIREBASE_AUTH "lBgsgqFp8m2xdcShRG7AgGqLZAaj90IY0sVDUKY2"

// Pin definitions
#define PH_SENSOR_PIN A0      // GPIO36
#define TDS_SENSOR_PIN A1     // GPIO39
#define TEMP_SENSOR_PIN 4     // GPIO4 for DS18B20
...
void loop() {
  ...
  sensorData["stationId"] = STATION_ID;
  ...
  sensorData["status"] = calculateStatus(phValue, tdsValue);
...
void configureFirebase() {
  ...
  // Set database read and write timeout
...
void sendToFirebase(JsonDocument& data) {
  String jsonString;
  serializeJson(data, jsonString);
  
  // Send to real-time path
  String realtimePath = "/realtime/" + STATION_ID;
  if (Firebase.setJSON(firebaseData, realtimePath, jsonString)) {
  ...
  if (Firebase.pushJSON(firebaseData, historicalPath, jsonString)) {
